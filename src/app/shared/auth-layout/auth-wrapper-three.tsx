...
import logoImg from "@public/logo_login_sf.png";
      ...
      <div className="relative flex min-h-screen w-full flex-col justify-center bg-gradient-to-tr from-[#136A8A] to-[#267871] p-4 md:p-12 lg:p-28">
        ...
        >
          <div className="flex flex-col items-center">
            <Link
              href={"/"}
              className="mb-7 inline-block max-w-[400px] lg:mb-9"
            >
              <Image src={logoImg} alt="Isomorphic" className="dark:invert" />
            </Link>
            <Title
              as="h2"
              className="mb-4 text-center text-[26px] leading-snug md:text-3xl md:!leading-normal lg:mb-5 lg:text-4xl lg:leading-normal"
            >
              {title}
            </Title>
          </div>
          {isSocialLoginActive && (
            <>
              <div className="flex flex-col gap-4 pb-2 md:flex-row md:gap-6 md:pb-2">
                <Button className="h-11 w-full" variant="outline">
                  <BsFacebook className="me-2 h-5 w-5 shrink-0 text-primary" />
                  <span className="truncate">Login com Facebook</span>
                </Button>
